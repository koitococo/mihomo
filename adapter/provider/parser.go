...
func ParseProxyProvider(name string, mapping map[string]any, tunnel C.Tunnel, topLevelProxyNames map[string]struct{}) (P.ProxyProvider, error) {
	...
	parser, err := NewProxiesParser(name, tunnel, topLevelProxyNames, schema.Filter, schema.ExcludeFilter, schema.ExcludeType, schema.DialerProxy, schema.Override, schema.AgeSecretKey)
	if err != nil {
		return nil, err
	}
